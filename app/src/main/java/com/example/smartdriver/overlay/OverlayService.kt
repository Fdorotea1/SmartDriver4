...
class OverlayService : Service() {
    ...
    // Location Manager para o Odómetro de fundo
    private var locationManager: LocationManager? = null
    private var backgroundLocationListener: LocationListener? = null
    ...
    private var finishPromptAlreadyShown: Boolean = false    // [NOVO] debitar popup por viagem
    ...
    override fun onCreate() {
        ...
        // Init do LocationManager para odómetro
    ...
    private fun setupTripTracker() {
        tripTracker = TripTracker(this, object : TripTracker.Listener {
            ...
            override fun onTripFinished(historyEntry: TripHistoryEntry) {
                // Parar odómetro e location updates
                stopBackgroundLocationUpdates()
                TripOdometer.stop()
                TripOdometer.reset()

                // [NOVO] limpar estado do popup ao fechar a viagem
                finishPromptAlreadyShown = false
                lastFinishVisibleState = false
                finishKeywordVisible = false

                OfferManager.getInstance(applicationContext).setTrackingActive(false)

                val effective = historyEntry.effectiveValue ?: 0.0
                // shiftSession.addTripEarnings(effective) // [REMOVED] Desativado
            ...
            override fun onTripDiscarded() {
                ...
                // [NOVO] limpar estado do popup ao descartar
                finishPromptAlreadyShown = false
                lastFinishVisibleState = false
                finishKeywordVisible = false
                ...
                // updateShiftNotification() // [REMOVED]
                updateIconPulseColor()
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        when (intent?.action) {
            ...
            ACTION_TOGGLE_SHIFT_STATE -> {
                if (intent.hasExtra(EXTRA_SHIFT_TARGET)) {
                    ...
                    if (shiftSession.isActive) {
                        ...
                        // startShiftTimer(); updateMenuViewShiftUI(); updateShiftNotification() // [REMOVED]
            ...
            // ACTION_SHOW_QUICK_MENU -> handleShowQuickMenu() // [REMOVED]
    ...
    private fun handleStartTrackingFromLast() {
        ...
        // Iniciar Odómetro Global e Location updates
        TripOdometer.start()
        startBackgroundLocationUpdates()

        // [NOVO] reset do estado do popup de fim de viagem para uma nova sessão
        finishPromptAlreadyShown = false
        lastFinishVisibleState = false
        finishKeywordVisible = false
    ...
    fun handleStartTracking(intent: Intent?) {
        ...
        // Iniciar Odómetro e Location Updates
        TripOdometer.start()
        startBackgroundLocationUpdates()
    ...
    private fun handleDiscardCurrentTracking() {
        ...
        // Parar Odómetro, Reset e Location
        stopBackgroundLocationUpdates()
        TripOdometer.stop()
        TripOdometer.reset()
    ...
    private fun handleHideOverlay() {
        // NEW: sempre que escondemos tudo, garantir que o mapa também desaparece
        forceHideTrackingMap()

        if (tripTracker.isTracking) {
            tripTracker.stopAndSave()
            // Se paramos a tracking, paramos odometro
            stopBackgroundLocationUpdates()
            TripOdometer.stop()
        }
    ...
    private fun handleShutdownRequest() {
        ...
        sendBroadcast(Intent("com.example.smartdriver.ACTION_SHUTDOWN_APP"))
        stopSelf()
    }
    ...
    /**
     * Força o esconder do mapa de tracking (fullscreen/header),
     * caso esteja aberto pelo TrackingOverlayView.
     */
    private fun forceHideTrackingMap() {
        runCatching {
    ...
    private fun hideTrackingOverlay() {
        // NEW: garantir que o mapa de tracking é fechado quando a bolha desaparece
        forceHideTrackingMap()
    ...
    private fun switchBubbleToIconUiOnly() {
        ...
        // NEW: ao sair da bolha, fecha também o mapa de tracking se estiver aberto
        forceHideTrackingMap()
    ...
    private fun updateFinishPromptForState(edgeOnly: Boolean) {
        val tracking = tripTracker.isTracking
        val shouldShow = tracking && finishKeywordVisible

        if (!tracking) {
            // Sem viagem em acompanhamento → garantir tudo limpo
            lastFinishVisibleState = false
            finishPromptAlreadyShown = false
            hideFinishConfirmPrompt()
            return
        ...
        if (edgeOnly) {
            // Modo “edge”: só reagimos OFF -> ON e NUNCA fechamos automaticamente
            // (evita que o popup pisque quando o keyword do Uber oscila).
            if (shouldShow && !lastFinishVisibleState && !finishPromptAlreadyShown) {
                showFinishConfirmPrompt()
            }
            lastFinishVisibleState = shouldShow
        } else {
            // Modo “full refresh”: usado em re-arm/boot do serviço
            lastFinishVisibleState = shouldShow
            if (shouldShow && !finishPromptAlreadyShown) {
                showFinishConfirmPrompt()
            } else if (!shouldShow) {
                hideFinishConfirmPrompt()
            }
    ...
    private fun showFinishConfirmPrompt() {
        ...
        // Se já há um diálogo visível, não fazer nada
        if (finishConfirmDialog?.isShowing == true) return

        // Se já disparámos um popup nesta viagem e o utilizador ainda não cancelou,
        // não voltar a abrir (debounce).
        if (finishPromptAlreadyShown) return
        ...
        val dialog = AlertDialog.Builder(this)
            ...
            .setNegativeButton("Cancelar") { d, _ ->
                // Utilizador não quis terminar agora → permitimos novo popup no futuro
                finishPromptAlreadyShown = false
                lastFinishVisibleState = false
                d.dismiss()
            }
            .setOnDismissListener {
                // libertar referência do diálogo
        ...
        try {
            ...
            // Marca que este popup já foi mostrado nesta viagem
            finishPromptAlreadyShown = true
        } catch (_: Exception) {
            finishConfirmDialog = null
            finishPromptAlreadyShown = false
        }
    ...
    private fun hideFinishConfirmPrompt() {
        try {
            finishConfirmDialog?.dismiss()
        } catch (_: Exception) { }
        finishConfirmDialog = null
        // Não mexemos em finishPromptAlreadyShown aqui:
        // - limpa-se no início da viagem
        // - ou quando o utilizador carrega em "Cancelar"
    }
    ...
    private fun applyZoneColorForOffer(offer: OfferData?) {
        ...
        var zoneName: String? = null // [NOVO] Nome da zona encontrada
        ...
        runCatching {
            ...
            if (hit != null) {
                ...
                // [NOVO] Extrair nome se disponível
                zoneName = hit.name
                Log.d("ZoneColor", "ZoneRuntime HIT → kind=$kindStr, name=$zoneName")
        ...
        if (kindStr == null) {
            ...
            if (zoneHit != null) {
                ...
                // [NOVO] Tentar ler nome via reflection
                zoneName = runCatching { readFieldOrGetter(zoneHit, "name")?.toString() }.getOrNull()
                ...
                Log.d("ZoneColor", "ZoneRepository → kind=$kindStr, name=$zoneName")
            } else Log.d("ZoneColor", "ZoneRepository: sem match")
        }

        // 3) Enviar estado E NOME ao OverlayView via broadcast
        val kindToSend = kindStr ?: "UNKNOWN"
        Log.d("ZoneColor", "APLICAR (broadcast) kind=$kindToSend, name=$zoneName")
        try {
            val i = Intent(com.example.smartdriver.overlay.OverlayView.ACTION_ZONE_HINT).apply {
                setPackage(packageName)
                ...
                // [NOVO] Enviar nome
                if (zoneName != null) {
                    putExtra(com.example.smartdriver.overlay.OverlayView.EXTRA_ZONE_NAME, zoneName)
                }
            }
            sendBroadcast(i)
        } catch (e: Exception) {
            ...
            (mainOverlayView as? com.example.smartdriver.overlay.OverlayView)?.updateZoneState(
                ...
                zoneName // Passar nome também no fallback
            )
        }
    ...
    // =================== Location updates para o Odómetro de fundo ===================
    ...
    private fun startBackgroundLocationUpdates() {
        ...
        if (backgroundLocationListener == null) {
            backgroundLocationListener = object : LocationListener {
                override fun onLocationChanged(loc: Location) {
                    // Só atualiza se a viagem estiver a decorrer
                    if (tripTracker.isTracking) {
                        TripOdometer.onLocation(loc.latitude, loc.longitude)
                    }
                }
        ...
        // Pedir updates ao GPS (ex: 2 segundos, 5 metros)
        try {
            ...
            Log.e("OverlayService", "Erro ao pedir location updates: ${e.message}")
        }
    }

    private fun stopBackgroundLocationUpdates() {
        if (backgroundLocationListener != null && locationManager != null) {
            try {
                locationManager?.removeUpdates(backgroundLocationListener!!)
            } catch (e: Exception) {
                Log.e("OverlayService", "Erro ao remover location updates: ${e.message}")
            }
            backgroundLocationListener = null
        }
    }
}
