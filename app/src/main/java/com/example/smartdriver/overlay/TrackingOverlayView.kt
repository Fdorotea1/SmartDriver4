...
class TrackingOverlayView(
        ...
        gestureDetector =
            GestureDetector(context, object : GestureDetector.SimpleOnGestureListener() {
                ...
                override fun onDoubleTap(e: MotionEvent): Boolean {
                    if (isDragging) return false
                    cancelDragDelay()
                    // Dois toques: parar acompanhamento (comportamento original)
    ...
    override fun onDetachedFromWindow() {
        // quando a bolha desaparece, garantir que o mapa de tracking não fica “perdido” aberto
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        // >>> cores do texto baseadas no rating <<<
        val topRating = initialHourRating ?: borderToIndividual(initialBorderRating)
        val bottomRating = currentHourRating.takeUnless { it == IndividualRating.UNKNOWN } ?: topRating
        val topColor = ratingToColor(topRating)
        val bottomColor = ratingToColor(bottomRating)

        drawTwoLinesWithLegends(
            canvas, cx, cy, radiusBase,
            topLegend, top, topColor,
            bottomLegend, bottom, bottomColor
        )
    }
    ...
    // garante que o mapa é fechado quando a bolha sai da janela (fim de viagem, swap para ícone, etc.)
